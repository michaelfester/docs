...
A basic _Scenario_ has two steps:
...
{% callout title="Gmail" %}
If you use Gmail with a _@gmail.com_ suffix, you may need to complete some additional steps. **Learn more.**
{% /callout %}
...
- The **Trigger**, _Watch Events_ (Stripe), checks for new events, such as new payments and new customers.
- The **Action**, _Add a Row_ (Google Sheets), will add a new row to Google Sheets every time new payments and new customers are created in Stripe.
